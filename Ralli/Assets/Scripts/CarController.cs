...
public class CarController : MonoBehaviour
{
    ...
    public float aiAccelerateSpeed = 1f, aiTurnSpeed = 8f, aiReacthPointRange = 5f, aiPointVariance = 3, aiMaxTurn = 15f;
    ...
    void Start()
    {
        ...
        if(isAI)
        {
            ...
            RandomiseAITarget();
            aiSpeedMod = Random.Range(0.6f, 0.7f);
        }
    ...
    void Update()
    {
        if (!RaceManager.instance.isStarting)
        {
            ...
            if (!isAI)
            {
                ...
                UIManager.instance.currenLapTimeText.text = string.Format("{0:00}m{1:00}.{2:000}s", ts.Minutes, ts.Seconds, ts.Milliseconds);
            ...
            else
            {
                ...
                if (Vector3.Distance(transform.position, targetPoint) < aiReacthPointRange)
                {
                    SetNextAITarget();
                }
            ...
            */
            ...
            if (grounded && (Mathf.Abs(turnInput) > .5f || (theRB.velocity.magnitude < maxSpeed * 0.5f && theRB.velocity.magnitude != 0)))
            {
                emissionRate = maxEmission;
            }

            if (theRB.velocity.magnitude <= .5)
            {
                emissionRate = 0;
            }
            ...
            if (skidSound != null)
            {
                if (Mathf.Abs(turnInput) > 0.5f && Mathf.Abs(theRB.velocity.magnitude) > 0.5)
                {
                    skidSound.volume = 1f;
                }
                else
    ...
    private void FixedUpdate()
    {
        ...
        transform.rotation = Quaternion.Euler(transform.rotation.eulerAngles + new Vector3(0f, turnInput * turnStrength * Time.deltaTime * Mathf.Sign(speedInput) * (theRB.velocity.magnitude / maxSpeed), 0f));
       // transform.rotation = Quaternion.Euler(transform.rotation.eulerAngles + new Vector3(0f, turnInput * turnStrength * Time.deltaTime * Mathf.Sign(speedInput), 0f));
    }

    public void ChekpointHit(int cpNumber)
    {
